...
public class OrderDAO {
    ...
    // ===== SAVE TO LAUNDRY LOGS =====
    public Order saveToLaundryLogs(Order order) throws SQLException {
        String sql = "INSERT INTO laundry_logs (customer_id, services, service_type, queue_number, weight, price, status) VALUES (?, ?, ?, ?, ?, ?, ?)";
        try (Connection conn = DatabaseConnection.getConnection();
             PreparedStatement pstmt = conn.prepareStatement(sql, Statement.RETURN_GENERATED_KEYS)) {
            pstmt.setInt(1, order.getCustomerId());
            pstmt.setString(2, order.getServices());
    ...
    public Order moveToDeliver(int queueNumber) throws SQLException {
        return moveOrderByQueueNumber(queueNumber, "to_fold_orders", "to_deliver_orders", "to_deliver");
    }
